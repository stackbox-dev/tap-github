...
def test_stargazers_forbidden_integration_is_treated_as_an_empty_stream() -> None:
    ...
    with patch.object(GitHubGraphqlStream, "request_records", side_effect=error):
        assert list(stream.request_records({})) == []


def test_stargazers_other_forbidden_errors_are_not_suppressed() -> None:
    """Only the known field-permission failure should be treated as empty."""
    stream = object.__new__(StargazersGraphqlStream)
    stream._logger = MagicMock()
    error = FatalAPIError("Graphql error: [{'type': 'FORBIDDEN'}]")
